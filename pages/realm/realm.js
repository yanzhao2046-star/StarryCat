/* ============================================================
   ...
   【坐标系 · 真正的 90³ 立方体网格】
    ...
    棋盘 12 × 12 = 144 格（A / B 各占 1 格，可落 142 格）
     BASE（关卡自带）：
       · A 起点  绿色半高台阶  90×90×45    初始 (0, 0)，顶面 z = 45   ← 可拖动，按 90 格步进挪位
       · B 终点  粉色半高台阶  90×90×45    在 (5, 2)，顶面 z = 45     ← 固定不可动
     A 是「可移动的固定块」：能拖走；其它块可以叠在它顶上（半高块叠 A = 拼成完整立方）
    A 被盖住时不可直接拖动，需先移开上面的块
     ...
     玩家拖出全部 9 件，从 A 顶（z=45）沿高差规则 BFS 到 B 顶（z=45）即为通关
...
/* 物料库分组定义 → 11 个独立块体（每个块体各自可被拖出 / 收回 / 用掉） */
...
Page({
  ...
  onLoad() {
    ...
    this.game = {
      ...
      pieces: expandTray(LEVEL.tray),   // 9 个独立块体：{ type, used }
  ...
  /* B 的实时锚点：B 也可拖动，标记要跟着走（与 startAnchor 同构）
     · 正常：取 board 上的 half_end
     · 正在拖 B：用预览落点
     · 都没有：兜底返回 LEVEL.end（关卡初始位置） */
  endAnchor() {
    const g = this.game
  ...
  /* 物料库显示：每种类型一格，显示剩余数量（顺序同 LEVEL.tray） */
  syncTray() {
    this.reconcilePieces()
    const tray = LEVEL.tray.map(g => ({
      type: g.type,
      total: g.count,
  ...
  applyEdgeSnap(type, fx, fy, fz) {
    ...
    /* 磁吸阈值：原 max(24, hw*0.6) 太敏感（远处就吸住）；改到 max(14, hw*0.32)，
       让「到很近再吸」更顺——和「棱线悬浮」精度同档（贴地歧义消解 0.85 不受影响） */
    const THRESH = Math.max(14, L.hw * 0.32)
    ...
    /* 贴地优先的歧义消解：悬空吸附（top / edge-*）投影被抬高了 sH，贴地拖拽时
       ...
       总被吸到棱线上。规则：悬空候选必须明显更近（dist < 0.85 × 贴地候选距离）
       才采纳，否则回落到贴地面候选 */
    if (best.z != null && bestFace && best.dist >= bestFace.dist * 0.85) {
      best = bestFace
    }
    if (DEBUG_TOUCH) {
  ...
  onReset() {
    ...
    g.pieces = expandTray(LEVEL.tray)
    g.drag = null
    g.preview = null
    g.won = false
    g.linked = false
  ...
  onShareAppMessage() {
    return { title: '毛布力猫不理 — 秘境', path: '/pages/realm/realm' }
  },
})
